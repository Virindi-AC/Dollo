...
module arm_movement(units){
	difference(){
        ...
        for (extrude=[0:units+3]){
            ...
            translate([21.5-95,21.5-95,extrude*30-45-5]) rotate([90,0,90]) wrap(units=units);
        }

	}
}

module finished(units){
	intersection(){
		translate([95,95,0]) arm_movement(units=units);
		hull(){
...
module base(units){
	intersection(){
		translate([0,0,0]) rotate([0,0,45]) finished(units=units);
		//translate([-50,-50,0]) cube([100,100,units*40]);
	}
}

module top_dove(units) {
    intersection() {
...
module twist_corner(units = 4){
    difference(){
        intersection(){
            rotate([0,-2,0]) base(units=units);
    ...
    translate([-2 * (units/2),0,0]) difference(){
        union(){
            translate([11,14.2,30*units+1.1]) top_dove(units=units);
            translate([-11,14.2,30*units+1.1]) top_dove(units=units);
        }
